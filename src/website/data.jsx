import { Facebook, Instagram, Twitter, Youtube } from "lucide-react";

export const recipes = [
  {
    recipe_aid: 1,
    recipe_title: "Sheet Pan Chicken Shawarma",
    recipe_slug: "sheet-pan-chicken-shawarma",
    recipe_published: "May 30, 2024",

    recipe_description:
      "This shortcut version of chicken shawarma skips the long marinating and roasting time and relies instead on quick cooking sliced thighs and a shower of the spices that go into the classic version--think cumin, cardamom and turmeric.  ",
    recipe_thumbnail: "shawarma.webp",
    recipe_time: "25mins",
    recipe_tag: "Easy",
    recipe_category: "chicken",
    recipe_author_image: "thumbnail.webp",
    recipe_author_name: "Song Ji Hyo",

    recipe_isFeature: true,
    recipe_isBanner: true,

    recipe_serving: 4,
    recipe_ingredients: [
      {
        title: "olive oil",
        measurement: 2,
        unit: "tbsp",
      },
      {
        title: "ground cumin",
        measurement: 1,
        unit: "tsp",
      },
      {
        title: "ground cinnamon",
        measurement: 1 / 2,
        unit: "tsp",
      },

      {
        title: "ground allspice",
        measurement: 1 / 4,
        unit: "tsp",
      },

      {
        title: "ground cardamom",
        measurement: 1 / 4,
        unit: "tsp",
      },
      {
        title: "ground turmeric",
        measurement: 1 / 4,
        unit: "tsp",
      },
      {
        title: "garlic, grated",
        measurement: 2,
        unit: "cloves",
      },
      {
        title:
          "boneless, skinless chicken thighs, sliced into 1-inch-thick pieces",
        measurement: 1,
        unit: "lb",
      },

      {
        title: "onion, halved and sliced into 1-inch-thick half-moons",
        measurement: 1,
        unit: "md",
      },

      {
        title: "full-fat Greek yogurt",
        measurement: 1 / 2,
        unit: "cup",
      },
      {
        title: "lemon",
        measurement: 1 / 2,
        unit: "pc",
      },
      {
        title: "lemon juice",
        measurement: 1 / 2,
        unit: "tsp",
      },
      {
        title: "pita bread",
        measurement: 4,
        unit: "tsp",
      },
      {
        title: "dill pickle spears, chopped",
        measurement: 3,
        unit: "tsp",
      },
      {
        title: "Roma tomato (about 6 ounces), chopped ",
        measurement: 1,
        unit: "tsp",
      },
      {
        title: "shredded lettuce ",
        measurement: 1,
        unit: "cup",
      },
    ],
    recipe_instruction: `
    \n ### Step - 1
    \n Preheat the oven to 450 degrees F. Lorem ipsum dolor sit amet.Lorem ipsum dolor sit amet consectetur adipisicing elit. Odit necessitatibus cupiditate laboriosam voluptates hic. Labore dolor vero voluptatem magnam, perspiciatis, porro saepe maxime, laboriosam cum nihil debitis dicta deserunt eius.
    \n ### Step - 2 
    \n Combine the olive oil, cumin, cinnamon, allspice, cardamom, turmeric, garlic, 2 teaspoons salt and a generous amount of pepper in a large bowl. Add the chicken and onions and toss to coat evenly. Spread in a single layer on a sheet pan, making sure the pieces are not overlapping. Bake until the chicken is cooked through and the onions are slightly caramelized, about 15 minutes.  
    \n ### Step - 3
    \n Meanwhile, mix the yogurt, lemon juice and a generous amount of pepper in a medium bowl until smooth. Warm the pitas in the microwave on a microwave-safe plate, 30 seconds to 1 minute.  
    \n ### Step - 4 
    \n Spread the yogurt sauce on top of the pitas and spoon the chicken and onions over the sauce. Top with the pickles, tomatoes and shredded lettuce.
    `,
    recipe_fats: "70g",
    recipe_sugar: "50g",
    recipe_sodium: "10g",
    recipe_carbohydrates: "20g",
    recipe_cholesterol: "40g",
  },

  {
    recipe_aid: 2,
    recipe_title: "Glazed Chicken and Broccoli Sheet Pan Dinner",
    recipe_slug: "glazed-chicken-and-broccoli-sheet-pan-dinner",
    recipe_published: "Sept 30, 2024",

    recipe_description:
      "Teriyaki chicken, crispy broccoli and creamy fingerling potatoes come together in this simple, satisfying sheet-pan dinner. ",
    recipe_thumbnail: "glazed-chicken.webp",
    recipe_time: "55mins",
    recipe_tag: "Easy",
    recipe_category: "chicken",
    recipe_author_image: "thumbnail.webp",
    recipe_author_name: "Song Ji Hyo",

    recipe_isFeature: true,
    recipe_isBanner: false,

    recipe_serving: 2,
    recipe_ingredients: [
      {
        title: "skin-on chicken drumsticks",
        measurement: 1,
        unit: "lb",
      },
      {
        title: "fingerling potatoes, halved lengthwise",
        measurement: 3,
        unit: "cup",
      },
      {
        title: "extra-virgin olive oil",
        measurement: 3,
        unit: "tsp",
      },

      {
        title: "medium head broccoli, cut into small florets",
        measurement: 4,
        unit: "cups",
      },
      {
        title: " purchased teriyaki glaze ",
        measurement: 2,
        unit: "tsp",
      },
      {
        title: "Kosher salt and freshly ground black peppe",
        measurement: "",
        unit: "",
      },
    ],
    recipe_instruction: `
    \n ### Step - 1
    \n Preheat the oven to 450 degrees F and line a rimmed baking sheet with parchment paper. Place the drumsticks on one side of the baking sheet, spacing them evenly apart. Place the potatoes on the other side of the baking sheet, leaving an empty space in the center for the broccoli. Drizzle the potatoes with 1 tablespoon of of the olive oil, and sprinkle with salt and pepper, tossing to evenly coat. Bake until the potatoes begin to soften and the drumsticks begin to cook and caramelize, 20 minutes.

    \n ### Step - 2 

    \n Remove the baking sheet from the oven and put the broccoli in the center. Drizzle the broccoli with the remaining 2 tablespoons of olive oil, and lightly sprinkle with salt and pepper, tossing with tongs to evenly coat. Brush the drumsticks with the teriyaki glaze on all sides.
 
    \n ### Step - 3
    \n Return the baking sheet to the oven and roast until the broccoli and potatoes are tender and charred in spots and the drumsticks are completely cooked through and read an internal temperature of 160 degrees F, 20 to 25 minutes more. Divide chicken and vegetables between two plates and serve.

    `,
    recipe_fats: "70g",
    recipe_sugar: "50g",
    recipe_sodium: "10g",
    recipe_carbohydrates: "20g",
    recipe_cholesterol: "40g",
  },

  {
    recipe_aid: 3,
    recipe_title: "Grilled Shrimp Scampi",
    recipe_slug: "grilled-shrimp-scampi",
    recipe_published: "Oct 1, 2024",

    recipe_description:
      "Lorem, ipsum dolor sit amet consectetur adipisicing elit. Eius tempora quisquam reiciendis saepe minus est, at eaque ex voluptate voluptatem!",
    recipe_thumbnail: "grilled-shrimp-scampi.webp",
    recipe_time: "50mins",
    recipe_tag: "Easy",
    recipe_category: "seafood",
    recipe_author_image: "thumbnail.webp",
    recipe_author_name: "Meggy Dawson",

    recipe_isFeature: false,
    recipe_isBanner: false,

    recipe_serving: 4,
    recipe_ingredients: [
      {
        title: "jumbo shrimp, peeled and deveined",
        measurement: 12,
        unit: "pc",
      },
      {
        title: "canola oil",
        measurement: 3,
        unit: "cup",
      },
      {
        title: "garlic",
        measurement: 10,
        unit: "cloves",
      },

      {
        title: "crushed red pepper flakes",
        measurement: 1 / 4,
        unit: "tsp",
      },
      {
        title: "ground fennel seed",
        measurement: 1,
        unit: "tsp",
      },
      {
        title: "stick unsalted butter",
        measurement: 1,
        unit: "stick",
      },
      {
        title: "large lemon, zested and juiced",
        measurement: 1,
        unit: "pc",
      },
      {
        title: "chopped tarragon leaves",
        measurement: 1,
        unit: "tbsp",
      },
      {
        title: "chopped fresh thyme leaves, plus sprigs for garnish",
        measurement: 2,
        unit: "tsp",
      },
      {
        title: "chopped fresh Italian parsley leaves, plus extra for garnish",
        measurement: 1 / 2,
        unit: "cup",
      },
      {
        title: "Kosher salt and freshly ground black peppe",
        measurement: "",
        unit: "",
      },
    ],
    recipe_instruction: `
    \n ### Step - 1
    \n Put the shrimp in a large baking dish.

    \n ### Step - 2
    \n Combine the canola oil, garlic, red pepper flakes and fennel seed in a small food processor and process until the garlic is somewhat paste-like. Pour this mixture over the shrimp and let them marinate for 30 minutes.
 
    \n ### Step - 3
    \n Light a grill to high heat.

    \n ### Step - 4
    \n Strain the shrimp from the marinade and season them with salt and black pepper, to taste. Grill the shrimp on both sides until they are slightly charred and just cooked through, about 2 minutes per side (see Cook's Note). Transfer the shrimp to a plate.

    \n ### Step - 5
    \n Melt the butter in a large saute pan set over the grates of the grill. Add the lemon zest, lemon juice, tarragon and thyme leaves and cook for 30 seconds. Add the shrimp and cook, tossing once, about 1 minute. Finish with the parsley and adjust the seasonings, if necessary. Transfer the shrimp to a recipe_serving platter and garnish with thyme and parsley.
    `,

    recipe_fats: "70g",
    recipe_sugar: "50g",
    recipe_sodium: "10g",
    recipe_carbohydrates: "20g",
    recipe_cholesterol: "40g",
  },

  {
    recipe_aid: 4,
    recipe_title: "Tuscan Chicken Skillet",
    recipe_slug: "tuscan-chicken-skillet",
    recipe_published: "Feb 1, 2024",

    recipe_description:
      "Bacon, cream, Parmesan -- is your mouth watering yet? There's no need to go out to dinner when you can make this restaurant-style creamy, cheesy chicken and fettuccine at home.",
    recipe_thumbnail: "tuscan-chicken-skillet.webp",
    recipe_time: "40mins",
    recipe_tag: "Easy",
    recipe_category: "pasta",
    recipe_author_image: "thumbnail.webp",
    recipe_author_name: "Meggy Dawson",

    recipe_isFeature: false,
    recipe_isBanner: false,

    recipe_serving: 4,
    recipe_ingredients: [
      {
        title: "fettuccine ",
        measurement: 12,
        unit: "oz",
      },
      {
        title: "garlic",
        measurement: 10,
        unit: "cloves",
      },

      {
        title: "bacon, chopped",
        measurement: 1,
        unit: "cup",
      },
      {
        title: "chicken tenders, cut into 1-inch pieces",
        measurement: 1,
        unit: "lb",
      },
      {
        title: "garlic, minced",
        measurement: 2,
        unit: "cloves",
      },
      {
        title: "plum tomatoes, chopped",
        measurement: 3,
        unit: "pc",
      },
      {
        title: "heavy cream",
        measurement: 1,
        unit: "cup",
      },
      {
        title: "baby spinach",
        measurement: 5,
        unit: "tsp",
      },
      {
        title: "grated Parmesan",
        measurement: 3 / 4,
        unit: "cup",
      },
      {
        title: "chopped fresh basil",
        measurement: 3,
        unit: "tbsp",
      },
      {
        title: "Kosher salt and freshly ground black peppe",
        measurement: "",
        unit: "",
      },
    ],
    recipe_instruction: `
    \n ### Step - 1
    \n Bring a large pot of salted water to a boil. Cook the fettuccine according to package directions; drain.

    \n ### Step - 2
    \n Meanwhile, put the bacon in a large, cold skillet, then cook over medium-high heat, stirring occasionally, until crispy, about 8 minutes; transfer to a plate with a slotted spoon. 

 
    \n ### Step - 3
    \n Sprinkle the chicken lightly with salt and pepper and add to the skillet in a single layer. Let cook, undisturbed, until golden brown on the underside, 2 to 3 minutes. Continue to cook, stirring occasionally, until cooked through, about 4 minutes more. Transfer to the plate with the bacon.


    \n ### Step - 4
    \n Reduce the heat to medium and add the garlic, stirring, until fragrant, about 30 seconds. Add the tomatoes and cream and bring to a simmer, then add the spinach and stir until just wilted. Add the bacon, chicken, fettuccine and Parmesan and toss with tongs until well coated; season to taste with salt and pepper. Sprinkle with basil and serve.
    `,

    recipe_fats: "70g",
    recipe_sugar: "50g",
    recipe_sodium: "10g",
    recipe_carbohydrates: "20g",
    recipe_cholesterol: "40g",
  },

  {
    recipe_aid: 5,
    recipe_title: "Fresh Corn Salad",
    recipe_slug: "fresh-corn-salad",
    recipe_published: "October 1, 2024",

    recipe_description:
      "Bacon, cream, Parmesan -- is your mouth watering yet? There's no need to go out to dinner when you can make this restaurant-style creamy, cheesy chicken and fettuccine at home.",
    recipe_thumbnail: "fresh-corn-salad.webp",
    recipe_time: "10mins",
    recipe_tag: "Easy",
    recipe_category: "salad",
    recipe_author_image: "thumbnail.webp",
    recipe_author_name: "Meggy Dawson",

    recipe_isFeature: false,
    recipe_isBanner: false,

    recipe_serving: 4,
    recipe_ingredients: [
      {
        title: "ears of corn, shucked ",
        measurement: 5,
        unit: "pc",
      },
      {
        title: " small-diced red onion (1 small onion)",
        measurement: 1,
        unit: "cup",
      },

      {
        title: "  cider vinegar",
        measurement: 3,
        unit: "tbsp",
      },
      {
        title: "good olive oil",
        measurement: 3,
        unit: "tbsp",
      },
      {
        title: "kosher salt",
        measurement: 1 / 2,
        unit: "tsp",
      },
      {
        title: "freshly ground black pepper",
        measurement: 1 / 2,
        unit: "tsp",
      },
      {
        title: "julienned fresh basil leaves",
        measurement: 1 / 2,
        unit: "cup",
      },
    ],
    recipe_instruction: `
    \n ### Step - 1
    \n In a large pot of boiling salted water, cook the corn for 3 minutes until the starchiness is just gone. Drain and immerse it in ice water to stop the cooking and to set the color. When the corn is cool, cut the kernels off the cob, cutting close to the cob.

    \n ### Step - 2
    \n Toss the kernels in a large bowl with the red onions, vinegar, olive oil, salt, and pepper. Just before recipe_serving, toss in the fresh basil. Taste for seasonings and serve cold or at room temperature.
    `,

    recipe_fats: "70g",
    recipe_sugar: "50g",
    recipe_sodium: "10g",
    recipe_carbohydrates: "20g",
    recipe_cholesterol: "40g",
  },

  {
    recipe_aid: 6,
    recipe_title: "Vegan Banana Bread",
    recipe_slug: "vegan-banana-bread",
    recipe_published: "October 1, 2024",

    recipe_description:
      "Bacon, cream, Parmesan -- is your mouth watering yet? There's no need to go out to dinner when you can make this restaurant-style creamy, cheesy chicken and fettuccine at home.",
    recipe_thumbnail: "vegan-banana-bread.webp",
    recipe_time: "1hr 50min",
    recipe_tag: "Easy",
    recipe_category: "vegan",
    recipe_author_image: "thumbnail.webp",
    recipe_author_name: "Meggy Dawson",
    recipe_author_bio:
      "This banana bread is eggless and dairy-free, but you'll hardly notice the difference. The crumb is moist and tender, and the flavor gets better each day (if it lasts!).",

    recipe_isFeature: true,
    recipe_isBanner: false,

    recipe_serving: 8,
    recipe_ingredients: [
      {
        title: "Cooking spray",
        measurement: 1,
        unit: "tsp",
      },
      {
        title: " cups unbleached all-purpose flour",
        measurement: 1.25,
        unit: "cup",
      },

      {
        title: "kosher salt",
        measurement: 1 / 2,
        unit: "tsp",
      },
      {
        title: " very ripe medium bananas, mashed",
        measurement: 1.25,
        unit: "cup",
      },
      {
        title: "recipe_sugar",
        measurement: 3 / 4,
        unit: "cup",
      },
      {
        title: "plain soy milk yogurt",
        measurement: 1 / 2,
        unit: "cup",
      },
      {
        title: "vegetable oil",
        measurement: 1 / 2,
        unit: "cup",
      },
      {
        title: "pure vanilla extract",
        measurement: 1,
        unit: "tsp",
      },
    ],
    recipe_instruction: `
    \n ### Step - 1
    \n Preheat the oven to 350 degrees F. Line the bottom and two long sides of a 9-by-5-by-3-inch loaf pan with parchment, leaving a 2-inch overhang on each long side, and lightly coat with cooking spray.

    \n ### Step - 2
    \n Whisk the flour, baking soda and salt together in a medium bowl. Whisk the bananas, recipe_sugar, yogurt, oil and vanilla together in another medium bowl. Gently fold the wet recipe_ingredients into the dry recipe_ingredients with a rubber spatula until the batter just comes together (it's OK if there are lumps). Transfer the batter to the prepared loaf pan and bake until a tester inserted in the center comes out clean, 55 to 60 minutes; tent the loaf with foil if it is browning too quickly. Cool the loaf in the pan on a wire rack for 30 minutes, then lift it from the pan by the parchment overhangs and cool completely on the rack. Store the loaf in an airtight container at room temperature for up to 5 days.
    `,

    recipe_fats: "70g",
    recipe_sugar: "50g",
    recipe_sodium: "10g",
    recipe_carbohydrates: "20g",
    recipe_cholesterol: "40g",
  },
];

export const socialMedia = [
  {
    icon: <Facebook strokeWidth={1} size={18} />,
    url: "https://www.facebook.com",
  },
  {
    icon: <Twitter strokeWidth={1} size={18} />,
    url: "https://www.twitter.com",
  },
  {
    icon: <Youtube strokeWidth={1} size={18} />,
    url: "https://www.youtube.com",
  },
  {
    icon: <Instagram strokeWidth={1} size={18} />,
    url: "https://www.instagram.com",
  },
];

export const navLinks = [
  {
    title: "Home",
    url: "/",
  },
  {
    title: "About",
    url: "/about",
  },
  {
    title: "Recipes",
    url: "/recipes",
  },
  {
    title: "Contact",
    url: "/contact",
  },
];

export const books = [
  {
    book_aid: 1,
    book_thumbnail: "book-1.jpg",
    book_url: "/book-1",
  },
  {
    book_aid: 2,
    book_thumbnail: "book-1.jpg",
    book_url: "/book-2",
  },
  {
    book_aid: 3,
    book_thumbnail: "book-1.jpg",
    book_url: "/book-3",
  },
];

export const videos = [
  {
    video_aid: 1,
    video_title: "video-1",
    video_url: "https://www.youtube.com/embed/jNAxSWczEj8?si=kTgxsyDWT13djYJM",
  },
  {
    video_aid: 2,
    video_title: "video-2",
    video_url: "https://www.youtube.com/embed/1q6qLcH8ADY?si=KGEunIvQSov3zTst",
  },
  {
    video_aid: 3,
    video_title: "video-3",
    video_url: "https://www.youtube.com/embed/WA7cHPB-94w?si=LcbHJIMcSi9fl_JW",
  },
];

export const categories = [
  {
    category_aid: 1,
    category_name: "Meat",
    category_slug: "meat",
    category_thumbnail: "shawarma.webp",
  },
  {
    category_aid: 2,
    category_name: "Chicken",
    category_slug: "chicken",
    category_thumbnail: "shawarma.webp",
  },
  {
    category_aid: 3,
    category_name: "Salad",
    category_slug: "salad",
    category_thumbnail: "shawarma.webp",
  },
  {
    category_aid: 4,
    category_name: "Pasta",
    category_slug: "pasta",
    category_thumbnail: "shawarma.webp",
  },
  {
    category_aid: 5,
    category_name: "Vegan",
    category_slug: "vegan",
    category_thumbnail: "shawarma.webp",
  },
  {
    category_aid: 6,
    category_name: "Seafood",
    category_slug: "seafood",
    category_thumbnail: "shawarma.webp",
  },
];

export const chefs = [
  {
    chef_aid: 1,
    chef_name: "Song Ji Hyo",
    chef_slug: "song-ji-hyo",
    chef_image: "song-ji-hyo.webp",
    chef_avatar: "song-ji-hyo-avatar.webp",
    chef_bio:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.",
    chef_fullbio: `
      \nLorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.
      \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.
      \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.
      \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.
      \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.`,
  },

  {
    chef_aid: 2,
    chef_name: "Meggy Dawson",
    chef_slug: "meggy-dawson",
    chef_image: "meggy-dawson.webp",
    chef_avatar: "meggy-dawson.webp",
    chef_bio:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.",
    chef_fullbio: `
    \nLorem ipsum dolor sit amet consectetur adipisicing elit. Quisquam, voluptatum.
    \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.
    \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.
    \nLorem ipsum dolor sit, amet consectetur adipisicing elit. Fugiat ipsum eligendi voluptates impedit eveniet eos, dolore ipsam enim voluptas consectetur dicta? Error reiciendis fuga quam quia esse quasi pariatur non nisi officiis nam iste fugit impedit, consectetur, minima officia natus.`,
  },
];
